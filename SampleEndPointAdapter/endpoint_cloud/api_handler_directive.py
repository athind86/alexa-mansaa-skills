...
class ApiHandlerDirective:
    ...
    def process(self, request, client_id, client_secret, redirect_uri):
        ...
        print("Shareer le lo maacho: ", request['body'])
        if json_body:
            ...
            if namespace == "Alexa.Authorization":
                ...
                if grantee_token == 'access-token-from-skill':
                    ...
                    # Get the User
                    auth_token = ApiAuth.get_token(grantee_token)
                    print('LOG directive.process.authorization.user_id:', auth_token)

                    # Get the Access and Refresh Tokens
                    api_auth = ApiAuth()
                    response_token = api_auth.get_access_token(auth_token, client_id, client_secret, redirect_uri)
